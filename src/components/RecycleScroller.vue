...
export default {
  ...
  methods: {
    ...
    itemsLimitError() {
      setTimeout(() => {
        console.warn(
          `[vue-virtual-scroller] It seems the scroller element isn't scrolling, so it tries to render all the items at once.`,
          "Scroller:",
          this.$el
        );
        console.warn(
          `[vue-virtual-scroller] Make sure the scroller has a fixed height (or width) and 'overflow-y' (or 'overflow-x') set to 'auto' so it can scroll correctly and only render the items visible in the scroll viewport.`
        );
      });
